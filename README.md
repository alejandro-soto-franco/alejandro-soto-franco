...
| [**stratum**](https://github.com/alejandro-soto-franco/stratum) | Research infrastructure CLI. `stratum cite` manages a local bibliography of arXiv / ISBN / Crossref entries with full-text OCR (MinerU + optional Nougat GPU sidecar) and math-aware BGE embeddings. `stratum cite lean-sync` ingests a Meridian RDF export into an oxigraph triple store for SPARQL-backed sorry analysis and dependency queries (`lean-query`, `lean-status`, `lean-forget`). `stratum cite lean-render` converts a Lean 4 declaration into journal-style LaTeX "putty" — Setting paragraph + lemma environment + one paragraph per `have` / tactic block — via a bundled `stratum-render.sty` macro package; dogfooded end-to-end on the `3d-navier-stokes` monograph. |
