...
| Project | Description |
|---------|-------------|
| [**Meridian**](https://github.com/alejandro-soto-franco/Meridian) | Lean 4 metaprogramming toolkit for mathematical formalisation. Sorry analysis with Mathlib `DiscrTree` coverage (available / partially available / not available), DOT dependency graphs, IDA* proof search with memoisation over the tactic state, and domain tactics for distributional PDEs (Sobolev exponent arithmetic, Biot-Savart, curvature, helicity) and geometric measure theory (varifolds, first variation, stationary varifolds, interior monotonicity). Apache 2.0. Runs entirely locally. |
